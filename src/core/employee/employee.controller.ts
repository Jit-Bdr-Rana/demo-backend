...
export class EmployeeController {
  ...
  @Post()
 async create(@Body() createEmployeeDto: CreateEmployeeDto,@Res() response:Response) {
    try{
 ...
 async update(
    ...
    try{
     ...
     return response.send({data:employee,message:'successfully update'}).status(201);
     }catch(e){
      response.send({data:null,message:'internal server error'}).status(500);
     }
  }
  ...
  async remove(@Param('id') id: string, @Res() response:Response) {
    ...
    try{
      const employee=await this.employeeService.remove(id);
          return response.send({data:employee,message:'successfully delete'}).status(201);
     }catch(e){
      response.send({data:null,message:'internal server error'}).status(500);
     }
  }
